...
**No-fail MCP/API/SDK/IDE connections:** A **no-fail condition** applies to checking **APIs, SDKs, IDEs, and MCP connections** and to **MCP additions to master MCP**. Every audit or integration cycle that touches these must: (1) check all connections and canonical alignment; (2) validate MCP additions via canonical-only and sync; (3) test after add or config change. The **ConnectedMCP** system (own repo: `connectedmcp` or `connected-mcp`) is self-improving, self-setup, self-configuration, integrated with 1Password, and uses Browserbase and/or other tools to discover/fetch MCP API/SDK when not in place and to test the same. Single reference: `docs/ONEWISHOS_NO_FAIL_MCP_CONNECTIONS_AND_CONNECTEDMCP.md`.
